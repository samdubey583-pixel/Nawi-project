...
function Guard({ role, children }: { role: Role; children?: React.ReactNode }) { const { user, loading } = useAuth(); if (loading) return <div className="loading">Loading secure workspace…</div>; if (!user) return <Navigate to="/login" />; if (user.role !== role) return <Navigate to="/" />; if (children) return role === 'REVIEWER' ? <ReviewerRoute>{children}</ReviewerRoute> : <>{children}</>; return role === 'TESTER' ? <AppShell><TesterDashboardWorkspace user={user} /></AppShell> : <ReviewerRoute><main className="dashboard"><h1>{role} workspace</h1></main></ReviewerRoute>; }
function ReviewerRoute({ children }: { children: React.ReactNode }) { const { user, logout } = useAuth(); return <ReviewerShell user={user} logout={logout}>{children}</ReviewerShell>; }
