...
function Home() { const { user } = useAuth(); const location = useLocation(); if (location.pathname.startsWith('/mobile/evidence/')) return <MobileEvidencePage />; return user ? <Navigate to={`/${user.role.toLowerCase()}/dashboard`} /> : <Navigate to="/login" />; }
