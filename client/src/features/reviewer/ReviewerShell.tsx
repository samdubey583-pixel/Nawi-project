...
import { reviewerSelectionStorageKey } from './reviewerSelection';
...
export default function ReviewerShell({ children, user, logout }: { children: React.ReactNode; user: any; logout: () => Promise<void> }) {
  ...
  const selectionKey = reviewerSelectionStorageKey(user?.id);
  const [rememberedTesterId, setRememberedTesterId] = useState(() => window.sessionStorage.getItem(selectionKey) || '');
  useEffect(() => {
    if (urlTesterId) { window.sessionStorage.setItem(selectionKey, urlTesterId); setRememberedTesterId(urlTesterId); }
    else if (choosingTester) { window.sessionStorage.removeItem(selectionKey); setRememberedTesterId(''); }
  ...
  return <div className="reviewer-shell"><AmbientShapes variant="dashboard" /><aside className="reviewer-sidebar"><div className="reviewer-side-brand"><Link className="reviewer-side-logo-link" to={`/reviewer/dashboard${context}`} aria-label="Review dashboard" title="Review dashboard"><Logo dark animated /></Link><small>TEST &amp; REPORT SYSTEM</small></div><nav aria-label="Reviewer workspace"><span className="reviewer-nav-label">REVIEWER WORKSPACE</span>{links.map(([label, Icon, to]) => { const active = isReviewerNavItemActive(label, location.pathname, searchParams); return <Link className={active ? 'active' : ''} to={to} key={label} aria-current={active ? 'page' : undefined}><Icon size={17} />{label}</Link>; })}</nav><div className="reviewer-side-account"><span className="reviewer-nav-label">ACCOUNT</span><b>{user?.firstName} {user?.lastName}</b><small>REVIEWER</small><button onClick={async () => { window.sessionStorage.removeItem(selectionKey); await logout(); navigate('/login'); }}><LogOut size={15} /> Sign out</button></div></aside><div className="reviewer-shell-main"><header className="reviewer-topbar"><span>REVIEWER WORKSPACE</span>{!onDashboard && <button type="button" onClick={() => navigate(`/reviewer/dashboard${context}`)}><ArrowLeft size={15} /> Go back</button>}</header>{children}</div></div>;
}
