...
export default function TareWorkspace() {
  ...
  const start = async (tareConfiguration?: Record<string, unknown>) => { setError(''); setSavingConfiguration(true); try { const response = await axios.post(`/test-reports/${reportId}/tare/start`, tareConfiguration ? { tareConfiguration } : {}); setState((current: any) => ({ ...current, report: response.data.report, applicability: response.data.applicability, test: response.data.test })); } catch (e: any) { setError(e.response?.data?.message || 'Unable to start Tare.'); } finally { setSavingConfiguration(false); } };
  const saveTareConfiguration = async () => {
    ...
    await start({ tareType: tareConfigurationForm.tareType, maximumTareEffect: { value: Number(tareConfigurationForm.maximumTareEffect), unit: tareConfigurationForm.maximumTareUnit }, tareOperationMode: tareConfigurationForm.tareOperationMode, tareWeighingDevicePresent: tareConfigurationForm.tareWeighingDevicePresent === 'Yes', presetTareDevicePresent: tareConfigurationForm.presetTareDevicePresent === 'Yes' });
  };
  ...
  const blocked = state.prerequisitesComplete === false;
  ...
  return <main className="tare-page">
    ...
    <section className="tare-content"><div className="tare-title"><div><span className="technical-label">TESTING · TARE</span><h1>Tare</h1><p>Review the report configuration before recording observations. Net-load MPE is determined from the actual evaluated net load.</p></div><span className="tare-status">{statusLabel}</span></div>
      ...
      {!test && state.applicability?.status !== 'NOT_APPLICABLE' && <>
        <section className="tare-configuration"><div><span className="technical-label">REPORT SNAPSHOT CONFIGURATION</span><h2>Tare test configuration</h2><p>{configurationRequired ? 'Complete these test-time characteristics to start A.4.6. They will be saved on this report only and will not change the reusable instrument record.' : 'These test characteristics are read from this report snapshot.'}</p></div>
