  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (phase.code === 'A.5.1') {
        ...
        const observations = voltageObservationPoints(test.plan, snapshot).map(point => ({ label: point.id, targetVoltage: point.targetVoltage, actualVoltage: Number(body[`${point.id}_actual`]), load: Number(body[`${point.id}_load`]), functionBehavior: body[`${point.id}_behavior`] || 'OPERATED', indication: num(body[`${point.id}_indication`]), deltaL: num(body[`${point.id}_deltaL`]), zeroError: num(body[`${point.id}_zeroError`]), unit: snapshot.unit || 'g' }));
...
function VoltageForm({ plan, snapshot, unit }: any) {
  ...
  return <>
    ...
    <section className="voltage-observations" aria-labelledby="voltage-observations-heading">
      ...
      <div className="voltage-sequence">{points.map(point => <article className="voltage-card" key={point.id}>
        <div className="voltage-card-heading"><div><span className="voltage-point-kicker">{voltagePointTitle(point.id)}</span><h5>{point.label}</h5></div><div className="voltage-target"><span>Target voltage</span><strong>{formatVoltage(point.targetVoltage)} V</strong><small>Read-only · OIML-derived</small></div></div>
