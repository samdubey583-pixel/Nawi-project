  ...
  return <main className="endurance-page">
    ...
    <section className="endurance-content">
      ...
      {test && applicability?.status === 'APPLICABLE' && <>
        ...
        <section className="endurance-panel">{test.postWeighing ? <><PrototypeNotice active={prototypeWorkflow} /><div className="endurance-saved"><Check size={16} /> Post-endurance weighing recorded · error {displayMass(test.postWeighing.Ec, unit)}</div></> : phase('A.6.3')?.status === 'AVAILABLE' ? <><PrototypeNotice active={prototypeWorkflow} /><PostWeighingForm test={test} unit={unit} busy={busy} onSave={(body: any) => void action('patch', '/post-weighing', body)} /></> : <div className="endurance-locked"><Lock size={16} /> Locked until the endurance loading phase is legitimately complete or explicitly skipped for prototype workflow testing.</div>}</section>
...
function PrototypeNotice({ active }: { active: boolean }) { return active ? <div className="endurance-prototype-notice"><strong>PROTOTYPE WORKFLOW — NOT LEGAL-METROLOGY ENDURANCE EVIDENCE</strong><span>This phase is being exercised using synthetic endurance workflow data. Its mathematical result is retained for software testing only and must not be represented as a genuine completed OIML endurance test.</span></div> : null; }
...
function CyclePanel({ test, phase, unit, plan, busy, action }: any) {
  ...
  const skipPrototype = async () => { const response = await action('patch', '/phase-02/skip-prototype'); if (response) setSkipOpen(false); };
  return <div className="cycle-panel">
    ...
    {test.cycleState === 'COMPLETED' && <div className="endurance-saved"><Check size={16} /> {syntheticCycles > 0 ? 'Synthetic prototype count reached 100,000. Actual laboratory evidence is still required.' : 'Exactly 100,000 applications recorded. Post-endurance weighing is available.'}</div>}
    {syntheticCycles > 0 && test.cycleState === 'COMPLETED' && !prototypeWorkflow && <><button className="endurance-secondary endurance-prototype-skip" onClick={() => setSkipOpen(true)} disabled={busy}>Skip Phase 02 — Prototype</button>{skipOpen && <div className="endurance-modal-backdrop" role="presentation"><div className="endurance-modal" role="dialog" aria-modal="true" aria-labelledby="prototype-skip-title"><span className="technical-label">PROTOTYPE WORKFLOW</span><h3 id="prototype-skip-title">Skip Phase 02 for prototype testing?</h3><p>This shortcut is for software demonstration and workflow testing only. It does not represent completion of the 100,000 laboratory endurance applications required for an actual OIML endurance test.</p><p>Synthetic endurance data will remain marked as prototype data.</p><div className="endurance-modal-actions"><button className="endurance-secondary" type="button" onClick={() => setSkipOpen(false)} disabled={busy}>Cancel</button><button className="endurance-primary" type="button" onClick={() => void skipPrototype()} disabled={busy}>Skip Phase 02 — Prototype</button></div></div></div>}</>}
