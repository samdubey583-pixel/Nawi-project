...
export default function StabilityOfEquilibriumWorkspace() {
  ...
  const prototypeFixture = data?.prototype === true;
  ...
  const needsRetestReset = data?.report?.status === 'RETEST_REQUIRED'
    && !test?.events?.some((event: any) => event.action === 'STABILITY_RETEST_ATTEMPT_RESET')
    && Boolean(test?.printStorage?.repetitions?.length || test?.zeroSetting?.repetitions?.length || test?.tare?.repetitions?.length);
  ...
  return <main className="stability-page">
    ...
    <section className="stability-content">
      ...
      {data.retestRequest && <section className="stability-condition stability-retest-banner" aria-label="Reviewer retest instructions"><div><strong>RETEST REQUIRED · Attempt {data.retestRequest.attemptNumber}</strong><p><b>Scope:</b> {data.retestRequest.targetPhaseCode || data.retestRequest.testCode}. {data.retestRequest.targetPhaseCode ? 'Only the requested phase is reopened; the other completed phases remain preserved.' : 'Repeat the requested A.4.12 procedure. The earlier attempt remains preserved in reviewer history.'}</p>{data.retestRequest.reason && <p><b>Reason:</b> {data.retestRequest.reason}</p>}{data.retestRequest.instructions && <p><b>Reviewer instructions:</b> {data.retestRequest.instructions}</p>}<small>Use the persisted request as the scope for this retest. Existing historical observations are not overwritten.</small></div></section>}
      {needsRetestReset && <section className="stability-condition stability-retest-reset"><strong>RETEST ATTEMPT NEEDS INITIALIZATION</strong><p>The previous submitted A.4.12 observations are preserved in retest history. Initialize a clean attempt before entering new observations.</p><button className="stability-primary" disabled={busy} onClick={() => void action(() => axios.post(`/test-reports/${reportId}/stability-of-equilibrium/retest/reset`))}>Begin clean A.4.12 retest</button></section>}
      ...
      <section className="stability-section"><span className="stability-kicker">A.4.12.1</span><h2>{prototypeFixture ? 'Prototype specification / configuration review' : 'Stability documentation / configuration review'}</h2><p>{prototypeFixture ? 'Record the synthetic prototype specification separately from real manufacturer documentation. This regression record does not claim a physical instrument was examined.' : 'Record manufacturer-provided information only. This review is separate from the physical stability observations.'}</p><div className="stability-check-grid">{documentationChecks.map(([field, label, options], index) => <label key={field}>{prototypeFixture && index === 0 ? 'Synthetic prototype specification available' : label}<select value={docs[field] || ''} onChange={event => setDocs({ ...docs, [field]: event.target.value })}><option value="">Select</option>{options.map(option => <option key={option}>{option}</option>)}</select></label>)}</div><div className="stability-fields stability-documentation-fields"><label className="stability-documentation-details">{prototypeFixture ? 'Synthetic prototype specification details' : 'Manufacturer documentation details'}<small>{prototypeFixture ? 'Describe only the modeled QA fixture assumptions. Keep the synthetic source explicit; do not present these details as a real manufacturer publication.' : 'Record the manufacturer-provided stable-equilibrium principle, criteria, adjustable and non-adjustable parameters, parameter security, and identified worst-case adjustment.'}</small><textarea value={docs[documentationDetailsField] || ''} onChange={event => setDocs({ ...docs, [documentationDetailsField]: event.target.value })} placeholder={prototypeFixture ? 'Enter the synthetic prototype stability specification and clearly identify it as regression data.' : "Enter the manufacturer-documented stability information here. Record only information supported by the manufacturer's documentation."} /></label><label>{prototypeFixture ? 'Prototype specification reference' : 'Manufacturer documentation reference'}<input value={docs.manufacturerDocumentationReference} onChange={event => setDocs({ ...docs, manufacturerDocumentationReference: event.target.value })} /></label><label>{prototypeFixture ? 'Synthetic evidence / provenance reference' : 'Evidence reference'}<input value={docs.evidenceReference} onChange={event => setDocs({ ...docs, evidenceReference: event.target.value })} /></label></div><button className="stability-secondary" disabled={busy || test.status === 'COMPLETED' || !completeDocs} onClick={() => void saveDocs()}>Save documentation review</button>{test.documentation?.reviewedAt && <span className="stability-saved-note">Documentation review saved · {test.phases.find((item: any) => item.code === 'A.4.12.1')?.result}</span>}</section>
