...
export default function ZeroSettingBeforeLoadingWorkspace() {
  ...
  const load = async () => {
    try { const response = await axios.get(`/test-reports/${reportId}/zero-setting-before-loading`); setState(response.data); setConfirmed(response.data.test?.zeroReferenceEstablished === true); setExecutionMode(response.data.test?.executionMode || 'PHYSICAL'); setNotes(response.data.test?.operatorNotes || ''); setProcedureObservations(response.data.test?.procedureObservations || { halfIntervalWeightApplied: false, indicationAlternatedAtZero: false, halfIntervalWeightRemoved: false, centreOfZeroReferenceReached: false }); setError(''); }
  ...
  const complete = async () => {
    setBusy(true);
    try { const response = await axios.post(`/test-reports/${reportId}/zero-setting-before-loading/complete`, { zeroReferenceEstablished: true, executionMode, operatorNotes: notes, ...(applicability?.method === 'A.4.3(a)' ? { procedureObservations } : {}) }); setState((current: any) => ({ ...current, ...response.data })); setConfirmed(true); setError(''); }
  ...
  return <main className="zero-setting-page">
    ...
    <section className="zero-setting-content">
      ...
      {applicability?.status === 'APPLICABLE' && <>
        ...
        {supported && test && !completed && <section className="zero-setting-panel zero-setting-confirm"><div><span className="technical-label">{nonAutomatic ? 'PROCEDURAL OBSERVATIONS' : 'CONFIRMATION'}</span><h2>{nonAutomatic ? 'Half-scale-interval zero-setting procedure' : 'Zero reference established before loading'}</h2><p>{nonAutomatic ? 'Following OIML R 76-1:2006 Annex A A.4.3(a), record each observed step. For prototype/regression execution, select synthetic simulation and make that classification explicit in the notes.' : 'The read-only A.4.2.3 result above is the zero deviation carried into subsequent weighing. Select the mode actually used. A synthetic simulation is recorded separately and must not be read as a physical procedure.'}</p></div>{nonAutomatic && <fieldset className="zero-setting-mode zero-setting-procedure-checks"><legend>Observed procedure steps</legend>{([['halfIntervalWeightApplied', 'Weights equivalent to half a scale interval placed on the load receptor'], ['indicationAlternatedAtZero', 'Indication adjusted until alternating between zero and one scale interval'], ['halfIntervalWeightRemoved', 'Half-scale-interval weights removed'], ['centreOfZeroReferenceReached', 'Centre-of-zero reference position attained']] as const).map(([key, label]) => <label key={key}><input type="checkbox" checked={procedureObservations[key]} onChange={event => setProcedureObservations(current => ({ ...current, [key]: event.target.checked }))} /> {label}</label>)}</fieldset>}<fieldset className="zero-setting-mode"><legend>Execution mode</legend><label><input type="radio" name="executionMode" checked={executionMode === 'PHYSICAL'} onChange={() => setExecutionMode('PHYSICAL')} /> Physical procedure</label><label><input type="radio" name="executionMode" checked={executionMode === 'SYNTHETIC_SIMULATION'} onChange={() => setExecutionMode('SYNTHETIC_SIMULATION')} /> Synthetic prototype simulation</label></fieldset><label className="zero-setting-check"><input type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)} /> {executionMode === 'PHYSICAL' ? 'Confirm the physical zero reference procedure was performed' : 'Confirm the synthetic A.4.3 workflow simulation was performed'}</label><label className="zero-setting-notes"><span>Operator notes {executionMode === 'SYNTHETIC_SIMULATION' ? '(required; identify synthetic simulation)' : '(optional)'}</span><textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder={executionMode === 'SYNTHETIC_SIMULATION' ? 'Synthetic prototype simulation; no physical instrument procedure performed.' : 'Record relevant procedural notes.'} /></label><button className="zero-setting-primary" onClick={complete} disabled={!confirmed || busy || (executionMode === 'SYNTHETIC_SIMULATION' && !/synthetic|simulation|regression/i.test(notes)) || (nonAutomatic && !procedureReady)}><Save size={16} /> Complete A.4.3</button></section>}
        {completed && <section className="zero-setting-panel zero-setting-complete"><Check size={25} /><div><h2>A.4.3 completed · {test.result}</h2><p>{nonAutomatic ? 'The recorded result is based on the saved A.4.3(a) procedural observations.' : `The A.4.2.3 zero deviation was carried forward as ${test.result}; no second measurement or compliance calculation was created.`} Execution mode: {test.executionMode === 'SYNTHETIC_SIMULATION' ? 'Synthetic prototype simulation' : 'Physical procedure'}.</p>{test.operatorNotes && <p className="zero-setting-source">Tester note · {test.operatorNotes}</p>}<Link className="zero-setting-primary" to={`/tester/reports/${reportId}/testing`}>Return to Test Route</Link></div></section>}
      </>}
      {error && <div className="zero-setting-error-message">{error}</div>}
      <section className="zero-setting-panel zero-setting-trace"><span className="technical-label">TRACEABILITY</span><p>{nonAutomatic ? 'OIML R 76-1:2006 Annex A A.4.3(a): half-scale-interval zero-setting procedure; recorded result reflects the four persisted procedural observations.' : 'OIML R 76-1:2006 Annex A A.4.3(b), using the completed A.4.2.3 result and its existing A.4.4.3 changeover/MPE calculation evidence.'}</p></section>
