...
type EvidenceContext = { reportId: string; testId?: string; subtestId?: string; instrumentId?: string; category?: string; evidenceType: string; label?: string; title?: string; testName?: string; oimlReference?: string };
...
export default function EvidencePanel({ context }: { context: EvidenceContext }) {
  ...
  const load = async () => { try { const response = await axios.get(`/reports/${context.reportId}/evidence`, { params: { testId: context.testId } }); setItems(response.data.evidence || []); } catch { /* Evidence remains unobtrusive if the report is unavailable. */ } };
  useEffect(() => { void load(); }, [context.reportId, context.testId]);
  ...
  return <section className="evidence-panel"><div className="evidence-heading"><div><span className="technical-label">SUPPORTING RECORD</span><h2>Evidence</h2><p>Optional supporting photographs and documents</p></div><button className="evidence-primary" onClick={() => { setOpen(true); setMessage(''); }}><ImagePlus size={16} /> Add Evidence</button></div>{message && !open && <div className="evidence-notice"><Check size={15} /> {message}</div>}{items.length > 0 && <div className="evidence-list">{items.map(item => <EvidenceRow item={item} key={item.id} onOpen={() => setViewer(item)} onDelete={() => void deleteItem(item)} />)}</div>}
    {open && <div className="evidence-modal-backdrop" role="presentation"><div className="evidence-modal" role="dialog" aria-modal="true" aria-labelledby="evidence-modal-title"><button className="evidence-close" onClick={() => { if (session) void cancelSession(); else setOpen(false); }} aria-label="Close"><X size={18} /></button><span className="technical-label">EVIDENCE CAPTURE</span><h2 id="evidence-modal-title">Add Evidence</h2><p>{context.label || definition.label}</p>{message && <div className="evidence-error">{message}</div>}{!session ? <><div className="evidence-choice-grid"><button className="evidence-choice" onClick={() => inputRef.current?.click()} disabled={busy}><FileImage size={20} /><strong>Upload From This Device</strong><small>Select an image from this computer.</small></button><button className="evidence-choice" onClick={() => void createMobileSession()} disabled={busy}><Smartphone size={20} /><strong>Use Mobile Camera</strong><small>Generate a temporary QR capture session.</small></button></div><input ref={inputRef} className="evidence-file-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { const file = event.target.files?.[0]; if (file) void uploadFile(file); event.currentTarget.value = ''; }} /><label className="evidence-note-field">Optional note<textarea value={note} onChange={event => setNote(event.target.value)} placeholder="What does this image show?" rows={3} /></label></> : <div className="evidence-qr-state"><img src={qr} alt="Scan this QR code with the phone camera" /><strong>{session.status === 'COMPLETED' ? 'Session completed' : message || 'Waiting for phone…'}</strong><span>{remaining ? `Expires in ${Math.floor(remaining / 60).toString().padStart(2, '0')}:${(remaining % 60).toString().padStart(2, '0')}` : 'Session expired'}</span><small>Scan with the phone camera. The phone is already bound to this report and test.</small><button className="evidence-secondary" onClick={() => void cancelSession()}>Cancel session</button></div>}</div></div>}
