...
export default function EvidencePanel({ context, readOnly = false }: { context: EvidenceContext; readOnly?: boolean }) {
  ...
  return <section className="evidence-panel"><div className="evidence-heading"><div><span className="technical-label">SUBMITTED EVIDENCE</span><h2>Evidence for this test</h2><p>{readOnly ? 'Evidence is locked with this submitted report.' : 'Optional supporting photographs and documents attached to this report and test.'}</p></div>{!readOnly && <div className="evidence-heading-actions"><button className="evidence-primary" onClick={() => { setOpen(true); setMessage(''); }}><ImagePlus size={16} /> {context.addLabel || 'Add Evidence'}</button><button className="evidence-secondary" onClick={() => { setOpen(true); setMessage(''); void createMobileSession(); }} disabled={busy}><Smartphone size={16} /> {context.mobileLabel || 'Capture from Phone'}</button></div>}</div>{message && !open && <div className="evidence-notice"><Check size={15} /> {message}</div>}{items.length > 0 && <div className="evidence-list">{items.map(item => <EvidenceRow item={item} key={item.id} onOpen={() => setViewer(item)} onDelete={() => void deleteItem(item)} readOnly={readOnly} />)}</div>}
...
function EvidenceRow({ item, onOpen, onDelete, readOnly }: { item: EvidenceItem; onOpen: () => void; onDelete: () => void; readOnly: boolean }) { return <article className="evidence-row"><button className="evidence-thumb" onClick={onOpen} aria-label={`Open ${item.label || 'evidence'}`}><img src={fileUrl(item)} alt="" /></button><button className="evidence-row-copy" onClick={onOpen}><strong>{item.label || item.originalFilename}</strong><small>{new Date(item.capturedAt || item.createdAt).toLocaleString()} {item.notes ? '· Note attached' : ''}</small></button>{!readOnly && <button className="evidence-icon-button" onClick={onDelete} aria-label="Delete evidence"><Trash2 size={15} /></button>}</article>; }
